...
app.controller('addFoundController',function($scope,$location,$timeout,$controller,addFoundService){
			...
			$scope.findNewest=function(page,size){
				...
				addFoundService.findNewest(page,size).success(
					function(response){
            ...
            $scope.$watch('entity.foundTime',function(newValue,oldValue){
            });
            /*手动输入限制 使用表单验证
          ...
          //日期控件引入结束
			...
			$scope.findAllCategory=function(){
				addFoundService.findAllCategory().success(
					function(response){
			...
			$scope.findAll=function(){
				addFoundService.findAll().success(
					function(response){
						$scope.list=response;
					}
				);			
			...
			$scope.findPage=function(page,size){
				addFoundService.findPage(page,size).success(
					function(response){
			...
			$scope.findOne=function(id){
				addFoundService.findOne(id).success(
					function(response){
				...
				if($scope.selectIds.length == 0){
					...
					layer.confirm('您确定要删除吗？', {
						  btn: ['确定','取消'] //按钮
							...
							addFoundService.del($scope.selectIds).success(
									function(response){
										if(response.status == 200){
											$scope.reloadList();//刷新
			...
			$scope.search=function(page,size){
				...
				addFoundService.search(page,size,$scope.searchEntity).success(
					function(response){
						$scope.list=response.rows;//显示当前页数据
						$scope.paginationConf.totalItems=response.total;//更新总记录数
					}
